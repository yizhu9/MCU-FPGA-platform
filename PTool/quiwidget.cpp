...
void QUIMessageBox::initControl()
{
    ...
    connect(btnMenu_Close, SIGNAL(clicked(bool)), this, SLOT(on_btnMenu_Close_clicked()));
...
void QUIDateSelect::closeEvent(QCloseEvent *)
{
...
void QUIDateSelect::on_btnOk_clicked()
{
    if (dateStart->dateTime() > dateEnd->dateTime()) {
        QUIHelper::showMessageBoxError("开始时间不能大于结束时间!");
        return;
    }
...
QByteArray QUIHelper::ushortToByteRev(ushort i)
{
    QByteArray result;
    result.resize(2);
...
void QUIHelper::showMessageBoxInfo(const QString &info, int closeSec)
{
    QUIMessageBox::Instance()->setMessage(info, 0, closeSec);
    QUIMessageBox::Instance()->show();
}

void QUIHelper::showMessageBoxError(const QString &info, int closeSec)
{
...
QString QUIConfig::PortName = "COM1";
int QUIConfig::BaudRate = 921600;
int QUIConfig::Mv = 0;
int QUIConfig::Width = 0;
int QUIConfig::Cycle = 1;
int QUIConfig::Point = 1;
bool QUIConfig::SaveLine = true;
...
void QUIConfig::readConfig()
{
    ...
    QUIConfig::PortName = set.value("PortName").toString();
    QUIConfig::Mv = set.value("Mv").toInt();
    QUIConfig::Width = set.value("Width").toInt();
    QUIConfig::Cycle = set.value("Cycle").toInt();
    QUIConfig::Point = set.value("Point").toInt();
    QUIConfig::SaveLine = set.value("SaveLine").toBool();
    set.endGroup();
}
...
void QUIConfig::writeConfig()
{
    ...
    set.beginGroup("BaseConfig");
    set.setValue("PortName", QUIConfig::PortName);
    set.setValue("BaudRate", QUIConfig::BaudRate);
    set.setValue("Mv", QUIConfig::Mv);
    set.setValue("Width", QUIConfig::Width);
    set.setValue("Cycle", QUIConfig::Cycle);
    set.setValue("Point", QUIConfig::Point);
    set.setValue("SaveLine", QUIConfig::SaveLine);
    set.endGroup();
}
